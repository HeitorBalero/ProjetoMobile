...
export default function Home({ navigation }) {
    ...
    async function deleteRefeicao(id) {
        try {
            await deleteDoc(doc(firestore, 'refeicao', id));
    ...
    useEffect(() => {
        const unsubcribe = onSnapshot(collection(firestore, 'refeicao'), (querySnapshot) => {
            const lista = [];
                            ...
                            <View style={estilo.refeicoesstyle}>
                                    <TouchableOpacity onPress={() => navigation.navigate("Alterar", {
                                        ...
                                        <View style={estilo.itens}>
                                            <Text style={estilo.titulo4}> <Text>Refeição Cadstrada</Text> </Text>
...
const estilo = StyleSheet.create({
    ...
    BtnCadastrar: {
        ...
        borderWidth: 2, // Adicione uma borda temporária
        borderColor: 'red', // Adicione uma cor de borda temporária
    },
     

    itensView: {
        height: '100%', // Tente ajustar para 100%
    },
